...
class TestJFKReveal:
    ...
    def test_generate_report(self, mock_report, temp_data_dir):
        ...
        # Setup mock
        mock_instance = MagicMock()
        mock_instance.generate_full_report.return_value = True
        ...
        mock_report.assert_called_once_with(
            data_dir=os.path.join(temp_data_dir["root"], "data/analysis"),
    ...
    @patch.multiple('jfkreveal.main.JFKReveal', 
                   ...
                   add_all_documents_to_vector_store=MagicMock(),
                   analyze_documents=MagicMock(),
                   generate_report=MagicMock(return_value=True))
    def test_run_pipeline_complete(self, temp_data_dir):
        ...
        jfk.process_documents.assert_called_once_with(
            max_workers=20, 
            vector_store=jfk.build_vector_database.return_value,
            skip_existing=False
        )
        jfk.add_all_documents_to_vector_store.assert_called_once_with(
            jfk.build_vector_database.return_value
        )
        jfk.analyze_documents.assert_called_once_with(
            jfk.build_vector_database.return_value
        )
    ...
    def test_run_pipeline_use_existing(self, temp_data_dir):
        ...
        jfk.scrape_documents.assert_not_called()
        jfk.build_vector_database.assert_called_once()
        jfk.get_processed_documents.assert_called_once()
        jfk.process_documents.assert_not_called()
        
        # Vector store add_all_documents should be called
        jfk.add_all_documents_to_vector_store.assert_called_once_with(mock_vector_store)
    ...
    @patch.multiple('jfkreveal.main.JFKReveal', 
                   ...
                   process_documents=MagicMock(),
                   analyze_documents=MagicMock(),
                   generate_report=MagicMock(return_value="dummy_report.html"))
    def test_run_pipeline_vector_store_failure(self, temp_data_dir):
        ...
        # A dummy report should be generated
        jfk.generate_report.assert_called_once()
        
        # Verify result
        assert result == "dummy_report.html" 